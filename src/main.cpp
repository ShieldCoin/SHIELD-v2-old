...
static CBigNum bnProofOfStakeLimit(~uint256(0) >> 8); // PoS is always scrypt
...
const string strMessageMagic = "SHIELD Signed Message:\n";
...
bool CTxMemPool::accept(CTxDB& txdb, CTransaction &tx, bool* pfMissingInputs)
{
    if (pfMissingInputs)
...
{
    int64 nRewardCoinYear = 0.00 * COIN;  // creation amount per coin-year
...
unsigned int ComputeMinWork(unsigned int nBase, int64 nTime)
{
    ...
    {
        // Maximum 300% adjustment per day...
        bnResult *= 3;
        nTime -= 24 * 60 * 60;
    }
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits, int algo)
{
    ...
    if (bnTarget <= 0 || bnTarget > limit) {
        printf("CheckProofOfWork() : nBits below minimum work for algo %s: %s > %s\n", GetAlgoName(algo).c_str(), bnTarget.getuint256().GetHex().c_str(), limit.getuint256().GetHex().c_str());
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in SHIELD
    bool fStrictPayToScriptHash = true; // Always active in SHIELD
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SHIELD", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
void PrintBlockTree()
{
    ...
    while (!vStack.empty())
    {
        ...
        printf("\nBLOCK: %d (%u,%u) %s %s %08x %08x %s mint %7s tx %" PRIszu"",
            ...
            pindex->nBlockPos,
            block.GetHash().ToString().c_str(),
            block.GetPoWHash(block.GetAlgo()).ToString().c_str(),
            block.nBits,
        ...
        if (tx.AcceptToMemoryPool(txdb, &fMissingInputs))
        {
            ...
            for (unsigned int i = 0; i < vWorkQueue.size(); i++)
            {
                     ...
                     ++mi)
                {
                    ...
                    if (tx.AcceptToMemoryPool(txdb, &fMissingInputs2))
                    {
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hashPoW > hashTarget && pblock->IsProofOfWork())
        return error("SHIELDMiner : proof-of-work not meeting target");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SHIELDMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("SHIELDMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadSHIELDMiner(void* parg);

static bool fGenerateBitcoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void SHIELDMiner(CWallet *pwallet, bool fProofOfStake)
{
    printf("CPUMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("SHIELD-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        printf("Running SHIELDMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadSHIELDMiner(void* parg)
{
    ...
    try
    {
        ...
        SHIELDMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadSHIELDMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadSHIELDMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadSHIELDMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d SHIELDMiner threads\n", nAddThreads);
