...
usage () {
	echo "USAGE: build-SHIELD.sh [-v version] [-s SIGNER] [-d true/false]"
	echo "Where v(ersion) should be the next version of the wallet"
	echo "Where s(igner) is used for the gitian signature, for example drakandar@ShieldCoin.github.io - no signer will skip signatures"
...
# Clone sources
git clone git://github.com/SHIELDTeam/fair-coin.git SHIELD
...
bin/gbuild ../SHIELD/contrib/gitian-descriptors/boost-linux.yml
mv build/out/boost-*.zip inputs/

bin/gbuild ../SHIELD/contrib/gitian-descriptors/deps-linux.yml
mv build/out/SHIELD-deps-*.zip inputs/

bin/gbuild ../SHIELD/contrib/gitian-descriptors/qt-linux.yml
mv build/out/qt-*.tar.gz inputs/

bin/gbuild ../SHIELD/contrib/gitian-descriptors/boost-win.yml
mv build/out/boost-*.zip inputs/

bin/gbuild ../SHIELD/contrib/gitian-descriptors/deps-win.yml
mv build/out/SHIELD-deps-*.zip inputs/

bin/gbuild ../SHIELD/contrib/gitian-descriptors/qt-win.yml
mv build/out/qt-*.zip inputs/

bin/gbuild ../SHIELD/contrib/gitian-descriptors/protobuf-win.yml
...
cd ..
git clone https://github.com/SHIELDTeam/gitian.sigs.git
